...
import {
  updateSingleTravelRequest,
  clearUpdateTravelRequest,
} from "../../redux/actions/updateTravelRequestAction";
import {
  clearSingleRequest,
  getSingleTravelRequest,
} from "../../redux/actions/singleTravelAction";
...
const filter = (travels, category) => {
  ...
  switch (category) {
    ...
    case "rejected":
      filteredArray = travels.filter((trav) => {
        return (
          trav.travelRequestInfo.status === "canceled" ||
          trav.travelRequestInfo.status === "rejected"
        );
      });
      return filteredArray;
    ...
    case "done":
      filteredArray = travels.filter((trav) => {
        if (trav.travelRequestInfo.Trip.length !== 0) {
          return (
            trav.travelRequestInfo.Trip[trav.travelRequestInfo.Trip.length - 1]
              .returnDate < Date.now() &&
            trav.travelRequestInfo.Trip[trav.travelRequestInfo.Trip.length - 1]
              .returnDate
          );
        ...
        <Container maxWidth="md" className={classes.cardContainer}>
          ...
          <Box>
            ...
            <div className={classes.paganete}>
              {filtered.length !== 0 && filtered.length / page_size > 1 && (
                <Pagination
